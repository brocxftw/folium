...
`.github/workflows/publish-images.yml` runs on `v*` tags: tests, amd64 build, compose smoke (`GET /health`), push to GHCR, upload Release assets (`docker-compose.yml`, `env.example`, `default.env.example`, installer tarball, `install-folium.sh`, `checksums.txt`). Packages must be made **public** once (see [install](install.md)).
