...
interface DocumentTagsControlProps {
  document: Document;
  stopPropagation?: boolean;
  /** Max tags shown before +N overflow (omit for all). */
  maxVisible?: number;
}

export function DocumentTagsControl({
  document: doc,
  stopPropagation,
  maxVisible,
}: DocumentTagsControlProps) {
  ...
  const visibleTags =
    maxVisible != null ? doc.tags.slice(0, maxVisible) : doc.tags;
  const overflow =
    maxVisible != null && doc.tags.length > maxVisible
      ? doc.tags.length - maxVisible
      : 0;
  ...
  return (
    ...
    >
      ...
      {overflow > 0 && (
        <span className="text-[11px] text-text-muted">+{overflow}</span>
      )}
